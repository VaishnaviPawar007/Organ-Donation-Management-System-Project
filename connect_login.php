...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $password = $_POST['PASS_WORD'];  // Do not trim passwords
    ...
    $stmt = $con->prepare("SELECT PASS_WORD FROM registration WHERE EMAIL = ?");
    ...
    if ($stmt->num_rows > 0) {
        ...
        if (password_verify($password, $hashed_password)) {
          header("Location: logged_in.php?EMAIL=" . urlencode($EMAIL));
            exit();
        } else {
